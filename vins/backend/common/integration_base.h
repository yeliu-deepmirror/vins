...
struct ImuIntrinsic {
  ImuIntrinsic() {}
  double acc_noise = 0.3;
  double gyr_noise = 0.06;
  double acc_random_walk = 0.04;
  double gyr_random_walk = 3e-3;
};
...
class IntegrationBase {
 public:
  ...
  void midPointIntegration(double _dt, const Eigen::Vector3d& _acc_0, const Eigen::Vector3d& _gyr_0,
    ...
    result_delta_q = delta_q * Eigen::Quaterniond(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2,
                                                  un_gyr(2) * _dt / 2);
  ...
  Eigen::Matrix<double, 15, 1> evaluate(const Eigen::Vector3d& Pi, const Eigen::Quaterniond& Qi,
    ...
    residuals.block<3, 1>(O_V, 0) =
        Qi.inverse() * (gravity_ * sum_dt + Vj - Vi) - corrected_delta_v;
