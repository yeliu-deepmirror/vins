...
System::System(const vins::proto::VinsConfig& vins_config) {
  ...
  estimator.SetParameter(Sophus::SO3d(qic), Eigen::Vector3d(vins_config.camera_to_imu().x(),
...
void System::PubImageData(double dStampSec, cv::Mat& img) {
  ...
  if (bPublish) {
    ...
    vector<set<int>> hash_vFeatureIds(feature::NUM_OF_CAM);
    for (int i = 0; i < feature::NUM_OF_CAM; i++) {
      ...
      for (unsigned int j = 0; j < vFeatureIds.size(); j++) {
        if (trackerData[i].vTrackCnt[j] > 1) {
          ...
          feature_points->id_of_point.push_back(p_id * feature::NUM_OF_CAM + i);
...
std::vector<std::pair<std::vector<ImuConstPtr>, ImgConstPtr>> System::getMeasurements() {
  ...
  while (true) {
    ...
    if (!(imu_buf.back()->header > feature_buf.front()->header + estimator.td)) {
      cerr << "wait for imu, only should happen at the beginning sum_of_wait: " << sum_of_wait
           << endl;
      sum_of_wait++;
      return measurements;
    }
...
void System::ProcessBackEnd() {
  ...
  while (bStart_backend) {
    ...
    for (auto& measurement : measurements) {
      std::cout << "==> [MEASUREMENT] process " << measurements.size() << " measurements."
                << std::endl;
      ...
      for (unsigned int i = 0; i < img_msg->points.size(); i++) {
        ...
        int feature_id = v / feature::NUM_OF_CAM;
        int camera_id = v % feature::NUM_OF_CAM;
      ...
      if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR) {
        ...
        cout << "==> [BACKEND] ProcessImage using: " << fixed << t_ProcessImage.toc()
...
Vector2d System::FindVelocity(IMG_MSG* last_features, int idx, double u, double v,
                              double dStampSec) {
  // if(last_feature)
...
void System::SavePoseAsTUM(std::string filename) {
  ...
  for (size_t i = 0; i < keyframe_history.size(); ++i) {
    ...
    save_points << t(0) << " " << t(1) << " " << t(2) << " " << q.x() << " " << q.y() << " "
                << q.z() << " " << q.w() << std::endl;
  }
}
